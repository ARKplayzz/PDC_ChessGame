...
public class Ranking 
{
    ...
    {
        double winnerElo = this.leaderboard.get(winner);
        double loserElo = this.leaderboard.get(loser);
        
        this.leaderboard.put(winner, (int)((double)this.leaderboard.get(winner) + 25 *(1 - this.predictElo(winner, loser))) );
        this.leaderboard.put(loser, (int)((double)this.leaderboard.get(loser) + 25 *(0 - this.predictElo(loser, winner))) );
        
        double[] t = {winnerElo, loserElo};
        return t;
    }
    
    private double predictElo(String player, String opponent)
    {
        return 1.0 / (1 + Math.pow(10, (this.leaderboard.get(player) - this.leaderboard.get(opponent)) / 400.0));
    }
